...
| **Any community-facing words** — public-page copy, headings, button labels, empty states, error text, alt text, metadata | **`/how-jesse-writes` skill** (the ratified guide; read `references/how-we-write.md` fresh — it drifts) + **`RIM_Public_Pages.md` → "Copy and voice"** for the web-specific rules. Non-negotiables: never **"free"** as a price word (it is *freely offered* / *community-supported*), never name the epoch, no em-dashes in prose, one image per page. **Ratification is Jesse's read-aloud — never inferred from silence.** |
...
- Per-page prefix system: `lp-` lessons, `pg-` programs, `wl-` welcome, `vol-` registrar, `adm-` admin, `db-` dashboard, `mp-` my profile, `nav-` nav, `hs-` host area, `hub-` hub components, `hh-` households, `ac-` account layout/sidebar, `ca-` course access.
