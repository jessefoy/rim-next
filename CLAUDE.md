...
| Zoom sessions — the "RIM orchestrates, Zoom is the room" migration (`app/session/[slug]/enter/*`, `lib/zoom.ts`, `lib/sessionMeeting.ts`, the `useZoom` / `recordByDefault` program flags, `/admin/zoom-test`) | **`RIM_Zoom.md`** — the per-tool reference (decision + account model, S2S provisioning, the per-occurrence meeting + self-heal, own-name hosting via Claim-Host, the no-registration/rate-limit pitfall, pilot & cutover) |
...
- No box-shadows. No borders unless functionally required. (Two scoped exceptions: (1) control-bar popovers in the LiveKit session room — `.rim-cb-popover` — a soft shadow because Zoom does, per the session-117 Zoom-aligned redesign; (2) white **cards** on the warm public-page ground use `--card-shadow`, a deliberately faint lift — session 148, see `RIM_Public_Pages.md`. Recede *panels* and everything else stay shadowless.)
