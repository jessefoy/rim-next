...
| Session room — **retired session 159** (the in-browser LiveKit room — `components/session/*`, `components/VideoRoom.tsx`, `app/api/livekit/*` — was removed; sessions run on Zoom, see the Zoom row) | **`RIM_SessionRoom.md`** — kept as historical reference + the DigitalOcean droplet server-ops (it still hosts OnlyOffice) |
...
| Document filing system — the per-hub Documents tab, the master directory (`/account/documents`), category governance, freshness/search, the placement/visibility filing model (`components/HubDocumentsClient`, `/account/hub/[slug]/documents/*`, `lib/documentAuth.ts`) | **`RIM_Documents.md`** — the filing-system design reference (hub → category → doc; search + sort + freshness; tended-not-gated categories; the cross-hub directory). Pairs with `RIM_OnlyOffice.md` (the office-editor machinery). |
| Mind Maps — the spatial brainstorming canvas + per-topic conversations (`/account/mindmaps`, the `/account/hub/[slug]/mindmaps` hub tab, `components/mindmap/*`, `lib/mindMapAuth.ts`, `lib/mindMapConversation.ts`, `/api/mindmaps/*`) | **`RIM_MindMaps.md`** — the per-tool reference (React Flow canvas; portable-resource model mirroring Documents; the per-map edit option; map-scoped conversations + notifications; floating edges, autosave serialization, the `@@unique` topic-thread rule) |
...
- Zoom — video conferencing for live sessions ("RIM orchestrates, Zoom is the room"; the self-hosted LiveKit room was retired session 159). The DigitalOcean droplet remains — it hosts OnlyOffice.
