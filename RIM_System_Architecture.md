...
**Gated surfaces:**
- LiveKit: `/api/livekit/token`, `/api/livekit/step-in`, `/api/livekit/mute-participant`, `/api/livekit/mute-all`, `/api/livekit/remove-participant`
...
### Office documents + the first hub-optional resource (session 154–155, live on prod, gated to coordinators)

Office documents (Word/Excel/PowerPoint) are edited in **self-hosted OnlyOffice**, not the RIM editor — live on production (gated to coordinators) and working end-to-end as of session 155 (create → edit → save → comment). The architectural shift: `HubDocument` is generalized so a document can live in **one hub, several, or none**. `HubDocument.hubId` is now nullable (hubless = community-project docs); cross-hub sharing is a `HubDocumentPlacement` join (one canonical doc surfaced in many hubs, never copied). A `docKind` enum (`NATIVE` / `ONLYOFFICE` / `LINK` / `UPLOAD`) and a `visibility` enum (`HUB` / `COORDINATORS` / `COMMUNITY`) layer on top. *The cross-hub/visibility UI + a master directory are Slice 4 — the model supports it, the UI isn't built yet; today every office doc is hub-origin (single hub, default `HUB` visibility).*

Consequently **document access is doc-level, not hub-level** — resolved by `lib/documentAuth.ts::canAccessDocument` (placements + visibility), the first resource in RIM not gated by a single hub. The OnlyOffice save callback (`/api/onlyoffice/callback`) is the **one deliberate non-session-gated route** in the hub-area API — authenticated by the shared-secret JWT instead, since the document server calls it server-to-server. (The s155 fix: that callback's JWT arrives in the `Authorization` header, which **nests the body under `payload`** — read `verified.payload ?? verified`, or `status`/`url` come back undefined and nothing saves.) Full model + infra + the integration gotchas: `RIM_OnlyOffice.md`.
