...
**Permission tiers (session 121, widened to Zoom-style in session 124).** The previous overloaded `isHost` flag was replaced with a three-tier model resolved by a single helper (`resolveSessionRole(userId, programSlug, sessionDate, roles)` → `{ isSessionHost, isCoHost, isHostTeam, isProgramTeacher }`). Every server route that gates a session-room action consults the same helper.

- **Session Host** — singular. `HostAssignment` for this exact session OR `ADMIN`. Token grant: `roomAdmin: true` + `canPublishSources` includes `SCREEN_SHARE`. Gates **End-for-All** and **Share Screen**. Tile renders the "Host" pill.
- **Co-host** — *widened in session 124*: any **active host-team `HubMember`** (`status="ACTIVE"` + `hostingCapability=true`) OR `HOST_MANAGER` OR `ProgramTeacher` for this program OR Session Host. ADMIN bypass. Token grant: `roomAdmin: true`, no screen-share source. Gates **mute-others / Mute All / Bell mode toggle / per-tile hover mute / Participants management**. No End-for-All. Tile renders the "Co-host" pill *only* when neither Host nor Teacher applies.
- **Teacher** — an *orthogonal identity* layered onto a person who has a `ProgramTeacher` row for this program. Always also Co-host (and may also be Session Host). Tile renders the "Teacher" pill alongside Host if both apply. Drives the bell-friendly `teacher` audio profile (NS off, AGC off, 128 kbps).
- **Participant** — everyone else, including guests. Token grant: `canPublishSources: [MICROPHONE, CAMERA]` only. UI hides Share / Mute-others / End-for-All; the LiveKit grant blocks screen-share publishing regardless of UI tampering. No pill.

**Why the widening matters.** The session-121 model required a plain `HOST` role member on the host-team hub to Step-In before they could mute, share, or use Bell mode — Co-host was reserved for `HOST_MANAGER` and `ProgramTeacher`. The session-124 widening recognizes that the host-team hub is already a curated, trained group with explicit membership state (`HubMember.status` + `hostingCapability`), and turning any active team member into a Co-host by default matches the Zoom-style "trust the team" model that the demographic actually expects. The hub authority gate remains the single source of truth — a coordinator can pause an individual via the hub admin UI and they correctly lose Co-host even if their role is still in place.
...
This decoupling matters operationally. The previous single-flag model granted End-for-All button visibility to anyone whose token had `isHost: true` (HOST_MANAGER, ProgramTeacher, and HostAssignment-match) while the server route silently rejected all but the assigned host — a button that did nothing when clicked. The tier split keeps the End-for-All authority where it belongs (the assigned host steward of the room) while still giving teachers and host managers and active team members full Co-host capability over mute/share/Bell mode.
...
**Session room features (Zoom-aligned redesign session 117, session 121 refinements, session 122 A/V tuning):** Bottom Zoom-style control bar (Mute · Start Video with device-picker chevrons → Participants · Chat → Share · Reactions · Settings · Bell mode (Co-host only) → red End with popover); Share Screen hidden for non-Co-hosts. Speaker / Gallery view toggle in the page header. Chrome stays visible at all times (auto-hide removed session 121). Participants panel: sticky local Me row with `Host` pill (Session Host only), raised hands floated to top, per-row mute (Co-host), Mute All footer, search at >10. Custom persistent chat with history + direct messages (server-persisted via `SessionChatMessage`, live via LiveKit data channel). Custom tiles with Zoom-style nameplate, active-speaker yellow outline, signal badges, initials-circle avatar fallback, and **hover-revealed Mute button on remote tiles for Co-hosts (session 121)**. Three-way audio profile (teacher / speaker / listener) driving capture defaults + publish bitrate (audio: 128 / 96 / 64 kbps; **video: 2.0 / 1.5 / 1.0 Mbps per profile as of session 122**). H.264 video at 30 fps with 3-layer simulcast `[h180, h360, h720]`. Pure-black background. **Krisp Enhanced Noise Cancellation on by default for every participant on join (session 122)** via `useKrispNoiseFilter()` from `@livekit/components-react/krisp`. **Bell mode** Co-host-only toggle in the control bar flips Krisp NC off so bells, singing bowls, and gongs pass through unfiltered; resets to NC-on at every join. Hidden when Krisp is unsupported in the browser. Greenroom carries a "Headphones recommended" line; Audio playback prompt for Safari with headphone hint. Emergency host step-in via header button (host-team members only) — stepping in upserts the HostAssignment, transferring Session-Host status to the stepper.
...
**Key files:** `lib/livekit.ts` (server SDK + `createRoomToken(permissions)`), `lib/livekitAuth.ts` (tier resolver, session 121), `app/api/livekit/token/route.ts`, `app/api/livekit/chat/route.ts` (history + DM filtering), `components/VideoRoom.tsx` (per-profile video bitrate ceilings + 3-layer simulcast, session 122), `components/session/RIMConference.tsx` (Krisp NC default-on hook wiring, session 122), `RIMControlBar.tsx` (Bell mode button, session 122), `RIMChat.tsx`, `RIMParticipantTile.tsx` (with hover-mute), `ParticipantsPanel.tsx` (with renamed `isCoHost` prop), `VideoSettingsPanel.tsx`, `DevicePickerMenu.tsx`, `ReactionsMenu.tsx`, `EndMenu.tsx` (End-for-All gated on `isSessionHost`), `ViewToggle.tsx`, `ControlBarIcons.tsx` (with `IconBell`, session 122), `components/session/sessionRole.tsx` (React context for tier distribution, session 121), `app/session/[slug]/page.tsx`. Detailed inventory in FEATURES.md §38; host/volunteer-facing changelog in `SESSION_ROOM_FOR_VOLUNTEERS.md`.
