...
**Session room features (Zoom-aligned redesign session 117):** Bottom Zoom-style control bar (Mute · Start Video with device-picker chevrons → Participants · Chat → Share · Reactions · Settings → red End with popover). Speaker / Gallery view toggle in the page header. Auto-hide chrome on idle (3s timer, `:has()`-driven overrides keep chrome visible when any panel/popover is open; touch devices never fade). Participants panel: sticky local Me row with `Host` pill, raised hands floated to top, per-row mute (host), Mute All footer, search at >10. Custom persistent chat with history + direct messages (server-persisted via `SessionChatMessage`, live via LiveKit data channel). Custom tiles with Zoom-style nameplate, active-speaker yellow outline, signal badges, initials-circle avatar fallback (when no presence photo). Three-way audio profile (teacher / speaker / listener) driving capture defaults + publish bitrate (128 / 96 / 64 kbps). H.264 video at 2.5 Mbps / 30 fps. Pure-black background. Audio playback prompt for Safari with headphone hint. Emergency host step-in via header button (host-team members only).

**Key files:** `lib/livekit.ts` (server SDK), `app/api/livekit/token/route.ts`, `app/api/livekit/chat/route.ts` (history + DM filtering), `components/VideoRoom.tsx`, `components/session/RIMConference.tsx`, `RIMControlBar.tsx`, `RIMChat.tsx`, `RIMParticipantTile.tsx`, `ParticipantsPanel.tsx`, `VideoSettingsPanel.tsx`, `DevicePickerMenu.tsx`, `ReactionsMenu.tsx`, `EndMenu.tsx`, `ViewToggle.tsx`, `ControlBarIcons.tsx`, `app/session/[slug]/page.tsx`. Detailed inventory in FEATURES.md §38; host/volunteer-facing changelog in `SESSION_ROOM_FOR_VOLUNTEERS.md`.
