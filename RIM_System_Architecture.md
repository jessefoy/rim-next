...
**Hub schema enhancements — sessions 73/167/168:** `HubStatus` enum (ACTIVE/ARCHIVED), `HubAppLink` for app installation/custom navigation, `HubAppLink.isPrimary` for the single app that leads a Space Home, and `firstVisitedAt` for newcomer welcome. The earlier `HubMember.activitySeenAt` Updates boundary was retired in favor of a stateless seven-day Recent view; its nullable column remains inert for migration safety. Migration `add_primary_to_hub_app_links` is additive, backfills the first enabled registered app by configured order, and adds a partial unique index on `hubId WHERE isPrimary=true`.
...
- **Universal Hub Home (sessions 167–168; presentation updated September 2026):** one `HubHomeClient` for every Space: stable core/app destination links, newcomer welcome, and named disclosures for personal attention, the primary app module, pinned conversations and coordinator-editable welcome/orientation. Shared history lives only in Updates. The module’s detailed content is not duplicated in a second card. `lib/toolRegistry.ts` declares each app's semantic navigation icon, compatibility, and promised contributions; the exhaustive server provider registry in `lib/hubApps.ts` fulfills Home, Updates, and attention without hub-slug branches.
...
- **Hub sidebar navigation (sessions 74/168):** Horizontal tab strip replaced with a 220px left sidebar that now collapses to a true 64px icon rail. Identity, Home, app links, Space sections, and settings retain distinct semantic icons; registered-app icon identity comes from `lib/toolRegistry.ts`. Mobile remains a full labeled drawer via hamburger. `HubNavStrip.tsx` and `HubHeader.tsx` were deleted in session 74.
...
**Entry permissions at the Zoom door (2026-09-24).** `/session/[slug]/enter` now enforces what My Home shows, instead of relying on the dashboard alone: a registration-required program (`!isOpenlyDroppable(kind, registrationEnabled)`) admits a member only with a registration that isn't CANCELLED, PENDING_PAYMENT or **WAITLISTED**, unless they are host-capable (`resolveSessionRole`), assigned to cover that day in any hub (including standing null-date rows), an ACTIVE member of the program's hosting hub or a `ProgramCoverageHub` hub, ADMIN/GT, or holding the program's open-access key. Resolved before provisioning, so a turned-away visitor never takes a seat. Every refusal is a plain-language page (`EnterNotice`), never a silent dashboard redirect. Scope routes by the **resource** (the program's hubs), per `RIM_Hub_Engineering.md` layer 1.
...
The Updates page (`/account/hub/[slug]/activity`, stable URL) remains a computed projection rather than a duplicate event ledger. `lib/hubActivity.ts` is the core query/serialization contract used by both the server page and paginated API; installed-app events arrive through `lib/hubApps.ts` providers. It merges conversation starts/replies, meaningful visible Google Files audit events, member joins, and app events. Every item carries `sourceKey`, `sourceLabel`, and `kind`; File comments are visibly distinct from Space Conversations. Filters are **All / Recent / For me**. Recent is a server-enforced rolling seven-day window and never changes merely because a page or item was opened. “For me” uses subscribed-thread replies, comments on files the viewer created, and app-owned relevance—not events authored by the viewer. A separate View organizer offers Date and time, All categories, and each exact category active in that Space; exact-source filtering happens before server pagination, while the category list derives from enabled core features and installed Updates-capable apps. Deleted threads and held-file draft events are excluded. Passive Updates have no unread rail state; durable Home attention is emitted only by an app with a live unresolved condition.
