...
### Office documents + the first hub-optional resource (session 154, branch — pending deploy)

Office documents (Word/Excel/PowerPoint) are edited in **self-hosted OnlyOffice**, not the RIM editor — spine built on `claude/onlyoffice-docs`. The architectural shift: `HubDocument` is generalized so a document can live in **one hub, several, or none**. `HubDocument.hubId` is now nullable (hubless = community-project docs); cross-hub sharing is a `HubDocumentPlacement` join (one canonical doc surfaced in many hubs, never copied). A `docKind` enum (`NATIVE` / `ONLYOFFICE` / `LINK` / `UPLOAD`) and a `visibility` enum (`HUB` / `COORDINATORS` / `COMMUNITY`) layer on top.

Consequently **document access is doc-level, not hub-level** — resolved by `lib/documentAuth.ts::canAccessDocument` (placements + visibility), the first resource in RIM not gated by a single hub. The OnlyOffice save callback (`/api/onlyoffice/callback`) is the **one deliberate non-session-gated route** in the hub-area API — authenticated by the shared-secret JWT instead, since the document server calls it server-to-server. Full model + infra: `RIM_OnlyOffice.md`.
