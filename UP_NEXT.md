...
### Session 163 (2026-07-14) — ✅ Google Workspace = RIM's document/file system: Slices 1–2 live on `main`; Slice 3 (writing) is next

**The decision (authority: `RIM_GoogleWorkspace.md`).** Google Workspace **replaces** native Tiptap documents as the primary doc/file system — "RIM orchestrates, Google is the file cabinet" (the Zoom pattern). One **service account** is RIM's only Google identity (Manager on each Shared Drive); **nobody gets a Google account**; RIM's DB is the permission system. Forks decided: **link-as-key** editing, **Drive folders are the filing system** (live-browsed, can't drift), **Community readable + editable by all members**. This deleted 2/3 of the source ChatGPT spec (no managed identities, no OAuth, no Groups/sync, no Admin SDK).

**Shipped + live (4 commits, deployed):**
- **Slice 1 (`ec6c0ed`)** — server-only `lib/google/` (RS256 JWT via `node:crypto`, **no new deps**); `Hub.googleDriveId/googleRootFolderId/googleFilesEnabled` + `GoogleFileAudit` (migration `google_workspace_foundation_v1`, additive); `/admin/hubs` Drive-mapping picker; `/admin/google-test` diagnostic (round-trip probes the org sharing policy).
- **Google-side setup done** — `rim-workspace` Cloud project, Drive API, `rim-files@…` SA (Claude drove the Console; Jesse held the credential steps). **Env `GOOGLE_SERVICE_ACCOUNT_EMAIL` + `GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY` set in Vercel** (an old Calendar-era EMAIL var was shadowing the key → "Invalid JWT Signature"; fixed + 3 dead Calendar vars removed via the Vercel CLI). `RIM — Community` Shared Drive exists with the SA as Manager. **Diagnostic passes end-to-end incl. the link-as-key sharing probe.**
- **Slice 2 (`509a234` + `3cd213f` + `caf5905`)** — the Finder: per-hub **Files** tab + `/account/files`, live folder browse (folders-first, breadcrumbs, phone drill-down), Google Docs read in-app (sanitized HTML → RIM typography on a white sheet), binaries streamed, editing via the gated link-as-key open route. Coexists with native Documents until cutover. New `gf-` CSS. **Two full 8-angle reviews** (25 findings fixed; security-heavy — Community exact-match, CSRF on the mint GET, reader self-gate, XSS-safe streaming, retry safety, URL-driven resync).

**OPEN — verify on prod (Jesse; Files is invisible to members until a hub is enabled):**
1. Account sidebar shows **Files** (you, as admin) → `/account/files` opens at **Community**.
2. Drop a PDF / create a Google Doc **in the `RIM — Community` Drive** → appears in the list; open the Doc → reads inside RIM on a white surface; "Open in Google Docs" opens the real editor in a new tab; folders drill down.
3. To preview a **hub** Files tab: `/admin/hubs → [hub] → edit` → pick its Drive + flip "Show the Files area" → that hub gains a Files tab.
4. Deploy log shows `google_workspace_foundation_v1`.

**NEXT — Slice 3 (the writing half):** "Create document" (Doc first; Sheets/Slides structured to follow), "New folder", rename, move, and **uploads** (small direct; large staged via Blob → `after()` transfer → cron backstop). Every write audit-logged (the `resolveDriveAccess`-returns-place work already wired `hubId`). **Before enabling Files on a real member-facing hub:** build the **admin link revoke/lockdown** action (backlog `2026-07-14-001`) — minted edit links are currently permanent with no revocation path. Then **Slice 4 (cutover):** migrate the few real native docs (HTML→Doc import), transfer Blob PDFs, retire the native editor.

**Deferred (backlog):** `2026-07-14-001` (admin revoke/lockdown — do before sensitive content), `2026-07-14-002` (enforce `googleRootFolderId` as an access boundary before mapping two hubs to one drive — latent today). Doc-fidelity note: the in-RIM reader deliberately drops Google's exact formatting (structure + emphasis only); "Open in Google Docs" carries fidelity. Jesse to feel out whether that's enough before considering an embed option.
...
- **LiveKit retired** — deleted `components/VideoRoom.tsx` + all `components/session/*` except `ZoomLaunch.tsx`, all 9 `app/api/livekit/*`, `/admin/livekit-test`, `public/noise/*`, and the **6 LiveKit/noise npm deps** (+144 transitive). Extracted the 2 SDK-free helpers → `lib/sessionIdentity.ts`; renamed `lib/livekitAuth.ts` → `lib/sessionAuth.ts` (the shared host-identity authority, now feeding the Zoom `canHost` gate). **`Program.useZoom` removed** from all code + schema (the reviewer caught that the original ADD migration would re-create a dropped column every deploy — removed it). The **physical DB column drop is deferred to a safe follow-up deploy** (backlog `2026-06-25-004`): `migrate.mjs` runs during the Vercel build against the shared prod DB, so a same-deploy `DROP COLUMN` would briefly 500 the still-live old code that selects it. `livekitRoom` column **KEPT** (now a vestigial-named "has a session" signal — rename is backlog `2026-06-25-001`).
...
- **Zoom scheduling integrity (Layers 1+2)** — from Jesse hitting a stale-meeting conflict after editing a program's time. **Layer 1:** the `programs-pg` PUT tears down a program's stale future meetings on a schedule change (guarded by `notBefore = now + EARLY_OPEN_MIN` so it never drops a staging host). **Layer 2:** new `lib/sessionConflicts.ts` — a recurrence-aware seat-conflict predictor (8-week look-ahead; overlap window matches the runtime seat-pick exactly) returned non-blocking as `seatConflicts` + a dismissible ProgramEditor banner. Reviewer-gated (its S1 = the early-open teardown guard). See `RIM_Zoom.md` → "Scheduling integrity."
- **Volunteer host docs** — refined `ZOOM_HOST_GUIDE.md` (Zoom: the host **types** their name); retired `SESSION_ROOM_FOR_VOLUNTEERS.md` to a tombstone; **gave Jesse two new docs in chat to place** ("Once You're Hosting", "What It Means to Host at RIM"). The in-app `host-session-room` manual chapter is LiveKit-stale — **reseed pending** (these docs are the source).
- **Docs** — CLAUDE.md + AGENTS.md orientation tables (session-room row retired, Zoom row updated; AGENTS.md gained the Zoom row it lacked), RIM_Zoom.md (+ the new integrity + guest-entry sections), RIM_SessionRoom.md (retired banner), RIM_System_Architecture.md, RIM_Stack_Reference.md, FEATURES.md, session-log.md; the `livekit-self-hosted` + `project-zoom-migration` memories updated to "cut over."
...
3. **Reseed the in-app `host-session-room` manual chapter** for Zoom (it's LiveKit-stale; the new markdown host docs are the source) — a `migrate.mjs` entry + deploy.

**OPEN — decisions / optional follow-ons:**
- **Guest link** — does RIM use external guests? It works + is persistent (the RIM `?key=` link wraps each occurrence's Zoom meeting). The only call: **keep + clarify the editor copy** vs **hide the share-link UI** as clutter.
- **Integrity view** — the standalone coordinator at-a-glance (next few weeks + any over-capacity slots), an optional follow-on to the save-time warning.
- Deferred backlog: column drop (`2026-06-25-004`), `livekitRoom` rename (`-001`), ProgramEditor structural perf (`-002`), CSS prune (`-003`), the seat-pick advisory lock (`2026-06-24-008`).

**Memory candidates (step 7b — proposed, awaiting Jesse's confirm):** (1) *feedback* — **two-phase column drops in this repo:** when dropping a DB column, also remove the original ADD-column `migrate.mjs` entry (or the idempotent re-runs fight), AND never `DROP COLUMN` a still-read column in the same deploy — `migrate.mjs` runs during the build against the shared prod DB, so the live old code 500s; ship reads/schema first, drop in a follow-up. (2) *feedback* — **check integrity at the moment of change, not only at creation:** a create-time check + a just-in-time runtime model leaves edits/overlaps unvalidated; pair a predictive check with a self-cleanup on edit (the seat-conflict work). (3) *feedback/user* — Jesse wants **forwardable, plain-speak volunteer docs** for the anxious non-reader (one action at a time, a calm net) — extends the s156 plain-English memory.
