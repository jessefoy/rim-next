...
**Session room three-tier permission model (session 121, widened session 124).** The previous overloaded `isHost` flag was split into Session Host + Co-host + Participant. One helper — `lib/livekitAuth.ts::resolveSessionRole(userId, programSlug, sessionDate, roles)` — returns `{ isSessionHost, isCoHost, isHostTeam, isProgramTeacher }` and is used by every server route that gates a session-room action (token, step-in, mute-participant, mute-all, end-session). Tier definitions:

- **Session Host** (singular) = `HostAssignment` match for this exact session OR `ADMIN`. Token grant: `roomAdmin: true` + `canPublishSources` includes `SCREEN_SHARE`. Gates End-for-All and Share Screen. Tile renders the "Host" pill.
- **Co-host** (session 124: widened to Zoom-style "trust the team") = active host-team `HubMember` (any role) with `status="ACTIVE"` and `hostingCapability=true`, OR `HOST_MANAGER`, OR `ProgramTeacher` for this program, OR Session Host. ADMIN bypass. Resolution consolidated into a single `getEffectiveHostingCapability(userId, "host-team", isManager || isProgramTeacher)` call where the role-based grant is the fallback when no `HubMember` record exists; an existing record is authoritative — if it's paused or revoked, the gate denies Co-host even for HOST_MANAGER and ProgramTeacher. Token grant: `roomAdmin: true`, no screen-share source. Gates mute-others / Mute All / Bell mode toggle / per-tile hover mute / Participants management. No End-for-All. Tile renders the "Co-host" pill *only* when neither Host nor Teacher applies (Host/Teacher take pill priority).
- **Teacher** (orthogonal identity, not a tier; session 124) = `ProgramTeacher` row for this program. Always also Co-host (and may also be Session Host). Renders the "Teacher" pill alongside Host if both apply. Drives the bell-friendly `teacher` audio profile (NS off, AGC off, 128 kbps).
- **Participant** = everyone else, including guests. Token grant: `canPublishSources: [MICROPHONE, CAMERA]` only. UI hides Share / Mute-others / End-for-All buttons. No pill.
...
- `host: true` ↔ Session Host (drives Host pill)
- `teacher: true` ↔ ProgramTeacher (drives Teacher pill)
- `cohost: true` ↔ has Co-host capability AND not Host AND not Teacher (drives Co-host pill)

The `cohost` constraint is enforced both server-side and in the client-side belt-and-suspenders `setMetadata` effect in `RIMConference.tsx`. A person can render at most two pills (Host + Teacher); never three. Trust note: `canUpdateOwnMetadata: true` (lib/livekit.ts) lets a client rewrite their own metadata, so the pills are UI cues only — real actions are gated server-side via `resolveSessionRole`.

**`isHostTeam` (Step-In gate, session 124).** Same hub-gate call with a narrower fallback (`isManager` only — ProgramTeachers don't gate Step-In because they teach in the room, they don't run it). Subtle change worth knowing: plain `HOST` role members without an active host-team `HubMember` record no longer see the Step-In button. The session-92 `syncHubMembership` flow creates the record on role assignment, so this should be a no-op in practice; flag if any host reports the button missing.
