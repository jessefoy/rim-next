...
_Generated 2026-03-11. Last updated 2026-05-20 (session 122 — LiveKit A/V tuning: Krisp Enhanced Noise Cancellation enabled by default for every participant via `@livekit/krisp-noise-filter` + `useKrispNoiseFilter` hook, per-profile video bitrate ceilings (teacher 2.0 / speaker 1.5 / listener 1.0 Mbps), explicit 3-layer simulcast `[h180, h360, h720]`, "Bell mode" Co-host toggle that flips NC off so bells/bowls/gongs pass unfiltered, "Headphones recommended" note in Greenroom; LiveKit Cloud tier corrected from "Ship" to "Build")._
