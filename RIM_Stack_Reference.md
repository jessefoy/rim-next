...
**Contextual Help System (session 61):** `ManualSection` model in `manual_sections` table — `slug @unique`, `title`, `hubSlug String?`, `body Json?`, `relations String[]`, `order Int`. Admin CRUD at `/admin/manual` (list), `/admin/manual/[slug]` (view), `/admin/manual/[slug]/edit` (editor). `ManualHelpIcon` component (`components/ManualHelpIcon.tsx`) — `?` link with `mh-` CSS prefix; wired into 9 staff pages. Seed scripts: `prisma/seed-manual.ts` (4 core sections), `prisma/seed-courses-manual.ts` (5 courses sections). The old hardcoded `/admin/manual/page.tsx` (~1200 lines) has been replaced with a DB-driven client component. **Closing ritual note:** After any session that adds, changes, or removes a feature, update the relevant `ManualSection` DB records via upsert on slug. This is part of every closing ritual, not optional.
