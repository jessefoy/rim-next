...
## The thank-you page (2026-09-24)

Stripe's `success_url` is **`/programs/[slug]/thank-you?session_id={CHECKOUT_SESSION_ID}`**. The page retrieves the Checkout Session from Stripe server-side and uses it only if its `metadata.programSlug` matches and `source` is `registration_dana`, so the amount, program and email can't be edited in the address. It says, in order: "You're registered for {program}", the thanks with the real amounts (gift, and any registration part), when and where, "two emails are on their way" (confirmation + receipt), how an online member joins (My Home), then one primary action (**Go to My Home** online; **Add to Google Calendar** in-person) with quiet calendar and program-page links. It trusts Stripe's `payment_status` rather than waiting for the webhook (which can land a few seconds after the redirect); an unconfirmable session gets a plain thank-you that claims nothing. `noindex`, `no-referrer`. Styles: `ty-` in `custom.css`.

The `cancel_url` still returns to the program page with `?dana=cancelled`. That message (and the legacy `?dana=success` one) used to render **under** the pull-quote card, which floats up with a negative top margin; both now render after it. The registration page was considered as the cancel target and rejected: a guest isn't signed in, so it can't recognize their existing registration and would offer a fresh form that ends in "Already registered."
...
- **The webhook re-reads each Checkout Session** (`stripe.checkout.sessions.retrieve`) before handling it, so the payload is always in the SDK's pinned API version (`lib/stripe.ts`). Stripe renders event bodies in the API version set on each webhook destination, and those differ (the original sandbox destination was 2013-02-13; its 2026 replacement, `rim-site-dana-2026`, is a "dahlia" release). A failed re-read returns 500 so Stripe retries. Webhook destinations must listen to `checkout.session.completed` and `checkout.session.expired`.
