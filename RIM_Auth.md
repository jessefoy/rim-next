...
6. **The three `/login` pages were rebuilt onto the `pp-` grammar in session 176** — the legacy Webflow class names (`.container-7-copy`, `.login-box`, `.form-header`, `.w-form`) are **gone**, and so is the near-black `#16192c` button and the pre-flip `#135274` teal they carried. They now use `pp-hero--flat pp-hero--short` + `.pp-form`, with `.pp-form__error` / `.pp-form__success` and a small `lg-` set. **The auth logic was not touched** by that rebuild: `handleSignIn` (including the DB-failure fail-safe and the not-member soft-redirect), `resendCode`, the post-`/join` warm-copy window, and `SignInCodeForm` are unchanged.

   **Two lessons from the old markup still govern these pages**, because the conditions that produced them have not changed:

   - The wrapper was `width: 100%` + 24px side padding with **no `border-box`**, so all three pages ran 48px past the viewport and the sign-in card sat 24px right of centre at any width under ~1148px (session 173). `.pp-form__input` and the `pp-` containers declare `border-box`, so the trap is closed — but this repo has **no global `* { box-sizing: border-box }`**, so anything new added here must declare it.
   - The code-entry page prints **the member's own address back to them**, which is a single unbreakable token: past ~34 characters it ran off the right edge, and `box-sizing` does nothing about it. It now carries `overflow-wrap: anywhere` via `.lg-email`. **The email arrives in a query param, so it has no safe upper bound** — any copy that echoes user-supplied text on these pages needs the same treatment.

   Detail + the measurement method: `RIM_Public_Pages.md` → "No global border-box."

7. **A `<form>` cannot live inside a `<p>`.** The resend action on `/login/check-email` is a real form sitting mid-sentence ("or send a new code"). Wrapping that sentence in a `<p>` is invalid HTML — the browser closes the paragraph early and the sentence visibly breaks apart. Its container is a `<div className="lg-alt">` for exactly this reason; caught in review during the session-176 rebuild.
