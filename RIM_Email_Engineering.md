...
- **Hub-NEUTRAL bodies** — for the per-session assignment/sub templates (`host-assignment-confirmation`, `host-assignment-removed`, `sub-request-claimed`) the cleanest fix was to drop the role word entirely ("confirmed for", "your session", "to someone else") rather than substitute a noun — the coverage `action` ("host this") reads awkwardly in most frames. `new-program-needs-host` is the exception: it gains `{{coverageNoun}}` ("may need AV coverage") because naming the role there is genuinely useful, and its builder passes `coverageNoun`. **Session 146** generalized this send to *every* scheduler hub via `notifyHubOfNewProgramCoverage(hubSlug, …)` (`lib/email.ts`): the primary host hub fires on program create (virtual/hybrid + `hostingRequired`), and each auxiliary hub (AV / greeter) fires when tagged — on create AND when newly added on edit (the `programs-pg` PUT diffs `coverageHubSlugs` against existing rows so re-saving the editor doesn't re-notify; removals stay silent). One template, each hub's own noun + `hubScopedUrl` link; recipients via `getHubNotificationRecipients`, so the pre-threshold gate applies automatically. The same heads-up across all hubs — the shared-base principle.
