...
| `Teacher` | `name`, `slug`, `bio Json?`, `photoUrl?`, `isActive`, `userId String? @unique` (optional User link) | ✅ Built; userId added session 66 |
| `LessonTeacher` | `id`, `lessonId`, `teacherId` (→ Teacher), `order Int`, `@@unique([lessonId, teacherId])`, `@@map("lesson_teachers")` | ✅ Re-migrated session 66 (was userId→User; now teacherId→Teacher) |
...
- **LessonTeacher model (session 66):** Re-migrated to use `teacherId String` → `Teacher` record. Session 65 had incorrectly landed on `userId → User`; session 66 corrects this to use the `Teacher` model as the join target. The `Teacher` model also gained an optional `userId String? @unique` field linking to a User account (set in the Teacher admin editor; not required). LessonEditor teacher picker now searches `/api/teachers/search` (Teacher.name). All lesson/series/courses pages updated to display `lt.teacher.name`. Migration required `prisma db push --accept-data-loss` (dropped userId column from lesson_teachers).
