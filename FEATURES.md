...
- **Community programs listing** — `/community-programs`. The public catalog of offerings, grouped by `ProgramCategory` (the taxonomy is editorial — changed in Program Manager, not in code). **Rebuilt session 170** onto the shared listing grammar: `pp-hero`, `.pl-card` rows that run the full container width and split into what-it-is (title + tagline) / when-it-is (schedule + format) / arrow, a 96 / 36 / 24 spacing cadence, and a closing membership block that speaks RIM's **dana** framing rather than "membership is free," linking to `/donate#dana-at-rim`. A hardcoded "Good first visit" badge (two slugs in the page file, never data) was removed in the same session.
...
- **Home** — `/`. **Rebuilt session 169** onto the `pp-` grammar with the live site's copy and section order (hero → Learn/Practice/Grow + Timeless Wisdom → Join a Community → the four program doors → Generosity). Looping bodhi-leaf video hero, dark and left-aligned — a **deliberate departure** from the live site's light centred hero, chosen by Jesse from an A/B. The old hardcoded seven-row "This Week at RIM" table was dropped; `/this-week` does it properly and the hero links there.
