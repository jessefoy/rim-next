...
| **Schedule & Location** | Category, date & time (display text), listing day+time, location + map link, meeting link, calendar datetime fields (`startDatetime` / `endDatetime`), recurrence fields (`recurrenceFreq` / `recurrenceInterval` / `recurrenceDays` / `recurrenceCount`) |
...
*Last updated: 2026-03-05 (session 28)*
