...
- **Updates** — `/activity`, a shared server projection of conversation, visible Google Files, member-join, and installed-app events. Every row names its source and event kind; File comments are distinct from Space Conversations. Filters are All / New / For me. A separate Recent / By category view pill preserves chronology as the default while dynamically grouping by core source or installed app when requested. `HubMember.activitySeenAt` keeps the quiet Updates-new dot independent from Home/conversation visits.
