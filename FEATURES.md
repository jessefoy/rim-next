...
- **Read (Slice 2)** — the Finder: per-hub **Files** tab + system-wide `/account/files` (`FilesBrowser`); folders-first browse, breadcrumbs, phone drill-down; in-app Google Doc reader (`/account/files/doc/[id]`); streamed binaries.
...
- **Access** — Community (reserved-name drive) readable + editable by every signed-in member; hub drives/folders require ACTIVE membership or GUIDING_TEACHER; ADMIN-alone does not pass (session-128 boundary). Env: `GOOGLE_SERVICE_ACCOUNT_EMAIL`, `GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY` (server-only).
- **Decided, not yet built (the finish sequence):** "everything is a **Space**" + UI rename to "Space"; **strict per-Space filing** — remove the global `/account/files` finder so files live only in a Space's context (Jesse's anti-"files everywhere" call); Community becomes a Space; then **Slice 4 cutover** (migrate native docs → Space folders, retire the native editor — the one-way door). **Cross-Space sharing deferred** (backlog `2026-07-15-001`). See `RIM_GoogleWorkspace.md` §9.
