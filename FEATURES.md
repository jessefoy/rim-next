...
| 2026-03-04 (session 21) | Calendar recurrence + Google Meet planning: Replaced `startDatetime`/`endDatetime`-only calendar support with full recurrence pattern system. Added `recurrencePattern` string-select field to Sanity programs schema (15 options: `daily_2`–`daily_5` for consecutive-day retreats/intensives, `weekly_2`–`weekly_12` for weekly courses, `monthly_3`/`monthly_4`/`monthly_6` for monthly series). `lib/calendarLinks.ts` gains internal `buildRRule()` (parses `"{freq}_{count}"` → RFC 5545 `RRULE:FREQ=DAILY/WEEKLY/MONTHLY;COUNT=N`) and exported `describeRecurrence()` (returns `{ googleLabel, icsLabel }` pair — e.g., daily_3 → `"first day only"` / `"all 3 days"`; blank strings for single events). Apple/Outlook `.ics` downloads include full RRULE so all sessions auto-populate in the calendar; Google Calendar URL always shows only the first session (RRULE not supported in GCal URL format) — labels make the distinction clear to members. Renamed Sanity `zoomLink`/`zoomLinkText` field titles to "Meeting Link" / "Meeting Button Text" + added descriptions forward-compatible with Google Meet. Updated all consumers: `lib/queries.ts` (replace_all), `app/api/programs/[slug]/ical/route.ts`, `app/api/registrations/route.ts`, `app/api/registrations/[id]/route.ts`, `app/programs/[slug]/page.tsx` (IIFE pattern to call `describeRecurrence()` in JSX). Clarified retreat design: continuous multi-day retreat (e.g. Fri eve → Sun afternoon) uses `startDatetime`/`endDatetime` spanning the full block — no RRULE needed; `recurrencePattern` is for same-time daily sessions. Added Section 19 (Google Meet Integration) to FEATURES.md — full spec, prerequisites, 4 code pieces, technical notes. Sanity deployed, TypeScript clean. Commits: 544ddb1, ad5e472. |
...
*Last updated: 2026-03-04 (session 21)*
