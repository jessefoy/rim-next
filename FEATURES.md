...
| `GUIDING_TEACHER` | Sangha-wide dharma authority. **Can enter and moderate every hub** — passes the hub access door (`lib/hubAuth.ts::canAccessHub`) without a membership row (session 135) — and acts as **implicit coordinator on every hub** for content + moderation: archive/restore/trash any document or thread, edit any thread, pin/unpin, override document lock, edit member status. Does NOT inherit ADMIN-level technical scope (hub config, hard-remove member, hub create/delete, system-wide). Assignable via the member registry under "Sangha-wide authority" (session 135). Scope: session 115. | Same as ADMIN if also ADMIN |
...
**Where roles are assigned:** Via the admin member detail page (`/admin/members/[id]`), in the Roles & Permissions section. Check or uncheck the role checkbox, then click "Save roles." No direct database access needed. Two groups (`components/member-sections/RolesSection.tsx`): **System Roles** (ADMIN, REGISTRAR, TEACHER, HOST, HOST_MANAGER) and **Sangha-wide authority** (GUIDING_TEACHER — added session 135 with a description of exactly what it grants; before that it was the one live role with no UI surface and could only be set via the DB console). The defunct `SUPPORT` role (Support Inbox, removed session 100) is intentionally not surfaced. The PATCH endpoint validates against the full `Role` enum and gates role changes to ADMIN.
...
> **Updated 2026-06-04 — the program-page CTA is now status-aware, keyed off offering kind.** The "what to do next" line in the Details card (upstream of the form) reads the full registration state instead of a binary on/off. The viewer's own standing shows first (✓ registered / on the waitlist — and it survives registration closing, fixing a bug where a registrant saw "Registration is closed" after the deadline); then program state ("Register →" / "Join the waitlist →" when full / "Registration is closed."). When registration is **off**, the offering's `ProgramCategory.kind` decides the meaning via `isOpenlyDroppable`: a drop-in / open community group shows format-aware "how to join" (and never a Zoom line on an in-person-only program), while a class / event / retreat shows **"Registration isn't open yet."** instead of the old misleading "Simply arrive in person." The ProgramEditor gained a read-only **"How this appears to visitors"** readout on the Registration tab + an inline "Kind: X" line on the Categories tab, so a registrar sees the result of their toggles at edit time (the gap behind LoriLee's report). No schema change. Full model: `RIM_Offering_Model.md`.
...
**Coming up for you logic:** Each `Registration` is projected to its program's next upcoming occurrence via `nextOccurrenceOnOrAfter()` (new in session 116, `lib/scheduleUtils.ts`) — walks forward up to 365 days from today's CT date, short-circuits for non-recurring programs with past anchors. Rows where `nextDateStr === null` (no future occurrence — completed series, past one-time programs) are filtered out. Rows where `nextDateStr === today` are filtered out (they're in Today above). Remaining rows sorted ascending by next date, sliced to 5. Each row's inline time is the start time projected to the next-occurrence date via `shiftToDate()`. **"Coming up for you" is registration-driven and NOT gated by offering kind** — anything you register for shows here, any kind (a registered community group like Qigong included).

**🔧 Session 137 — kind-aware Today placement + recurrence fix.** Two things landed here:
- The dashboard now consumes the shared `isOccurrenceOnDate` (its old local `isOccurrenceToday` copy was removed in the helper-consolidation), so it inherits the **session-137 recurrence fix**: `isOccurrenceOnDate` previously treated `endDatetime` as a series-end cutoff for recurring programs, which erased every recurring program from "Coming up," `/this-week`, the Scheduler, host rotations, and the session-join gate. Fixed in `lib/scheduleUtils.ts` by scoping the cutoff to non-recurring programs. See `RIM_Scheduler.md` pitfall.
- **Today** now filters by **offering kind** (`ProgramCategory.kind` via `lib/programKind.ts::isOpenlyDroppable`): a public "Join now" shows only for openly-droppable kinds (`DROP_IN` always; `COMMUNITY_GROUP` when registration is off) plus the viewer's own registered/hosted sessions (ADMIN sees all). A registration-required `CLASS`/`EVENT`/`RETREAT` no longer invites a non-registrant to join — it surfaces in their "Coming up for you" once registered. The per-session `findFirst` registration check was replaced by one batched `todayRegSlugs` lookup. See FEATURES §50 + `RIM_Offering_Model.md`.

**Key files:** `app/account/(authenticated)/dashboard/page.tsx`, `components/DashboardAutoRefresh.tsx`, `lib/scheduleUtils.ts`, `lib/programKind.ts`
...
## 38. LiveKit Video Conferencing — Phases 1–5 ✅ Built — sessions 76, 86, 117 (Zoom-aligned redesign), session 121 (three-tier permission model + cleanup), session 122 (Krisp NC + per-profile video bitrate + Bell mode), session 124 (full audit, Zoom-style tier widening + three visible pills, Krisp instrumentation, Step-In propagation + timing fix, ProgramTeacher backfill), session 125 (identity vs. capability split, Host Volunteer rename, raised-hand speaking queue, persistent vote signals), session 126 (server-side time gate on the token route + per-session rooms with per-session chat scoping), session 127 (per-program teacherLabel — Teacher / Guide / Facilitator / Instructor / Custom), session 128 (Silent Meditation Hub Slice 1 — hub-grants-teacher capability path + ProgramEditor "Hosting & Access" tab restructure; Slice 2 — hub operational + ADMIN no longer bypasses hub content access + Add-me-as-coordinator admin affordance; Slice 2.5 — hub-scope every outbound email URL + welcome-email reliability fix + canonical CTA button helper + three engineering reference docs; Slice 2.6 — standing-rotation system generalized to be hub-aware), session 133 (session-room UX batch: Bell-mode label clarity, device chevrons removed, DM-by-name + unread-chat badge, join muted/dark by default, local Pin, fullscreen screen share + pre-share primer, full names on tiles/roster/chat)

### Session-room UX batch (session 133)

A clarity-and-parity pass on the live room, shipped as four reviewer-gated slices + a follow-on. **Engineering reference: `RIM_SessionRoom.md`** (created this session).

- **Bell mode label** (`RIMControlBar`) — stable "Bell mode" label with a gold highlight + "On" marker when active. Previously the label flipped to "Clean voice" *while in bell mode*, which read backwards. Bell mode ON ⇔ Krisp NC off; aria-pressed/highlight/marker all key off the same condition.
- **Device chevrons removed** — the inline mic/camera chevron popovers duplicated the Settings panel (`VideoSettingsPanel`, which fully owns mic/speaker/camera selection) and read as dead controls. `DevicePickerMenu` deleted; mic/cam are plain buttons; off-state red tint via `.rim-cb-btn--off`.
- **DM by clicking a name** — `ParticipantsPanel` gained `onMessageParticipant`; clicking a remote participant's name closes the roster, opens chat, and pre-targets a private message. The chat `recipient` was lifted from `RIMChat` to `RIMConference` and `RIMChat` made controlled.
- **Unread-chat badge** — `RIMControlBar` already rendered a badge but nothing fed it; an always-on `DataReceived` listener in `RIMConference` counts `CHAT_TOPIC` packets received while the panel is closed (own sends never counted — LiveKit doesn't loop `publishData` to the sender; DMs to others never reach this client), resets on open, capped "9+". `CHAT_TOPIC` exported from `RIMChat`.
- **Join muted + camera off** — `Greenroom.acquireMediaPermission()` acquires the camera/mic grant via `getUserMedia` + immediate `stop()` (never publishes a frame — "join unseen"), so the user lands silent/dark and turns each on instantly when ready (grant primed, no second prompt). Preserves the iOS-Safari gesture chain + denial→Recovery. `Status` enum renamed `auto-acquiring`/`acquiring`.
- **Local Pin** — hover a tile → **Pin** (bottom-right) keeps that person as the viewer's main view regardless of active speaker; personal/local, not broadcast. `sessionRole` context carries `pinnedIdentity` + `onTogglePin`; pin banner gives a visible Unpin. Precedence: manual pin > screen share > speaker-follow > gallery.
- **Fullscreen screen share** — a published `ScreenShare` track auto-focuses for everyone (was a small tile); camera tiles drop to the filmstrip; livekit-prefabs letterboxes it (`object-fit: contain`). New `ShareScreenPrimer` frames the browser's own (unstylable) picker before it opens. Speaker-view "keep pin" guards require a Camera source so a stopped share can't leave a blank pin.
- **Full names** — tiles, roster, and chat sender names show first + last via `lib/livekit.ts::sessionDisplayName` (`(preferredName || firstName) + lastName`), wired into the token + chat routes. Global `session.user.name` stays first-name-only.

**Files:** `components/session/{RIMConference,RIMControlBar,RIMChat,ParticipantsPanel,RIMParticipantTile,Greenroom,sessionRole,ControlBarIcons}.tsx`, new `components/session/ShareScreenPrimer.tsx`, deleted `components/session/DevicePickerMenu.tsx`, `components/VideoRoom.tsx`, `lib/livekit.ts`, `app/api/livekit/{token,chat}/route.ts`, `public/css/custom.css`, manual chapter `host-session-room` v10 (`prisma/update-manual-host-session-room.mjs` + `prisma/migrate.mjs`). Commits `232973e`, `8021388`, `6c929c2`, `f7d4517`, `acb8650`.

**Deferred:** latency/sync tuning (items 2 + 7 — needs a live measurement pass, parked per Jesse); mobile pin-from-tile (hover-only today); the sharer's own blank focus tile during a whole-screen share; a guest full-name nudge.
...
**Same-day follow-ups (commits `11864f2` → `93f985e` → `3117833`).** Real-world testing immediately after the first ship surfaced two deeper issues the original pass missed.

Diagnostic patch first (`11864f2`): when Jesse reported the per-program Reset wasn't working on multi-day programs without a screenshot, I couldn't pin the failure from code alone. Shipped a self-diagnosing patch — `console.log` under `[reset]` on client + `[reset-rotations]` on server + an inline result line at the click point (the page-level toast renders at the top of the rotations area; if you're scrolled to the bottom of a program card the toast is off-screen and the action looks like it did nothing).

Heal migration + atomic transfer (`93f985e`): root cause located via the diagnostic patch. Orphan `StandingAssignment` rules on hubs that no longer matched the program's `hostingHubSlug` (programs that had been transferred between hubs after rotations were set up). The apply cron walks every rule regardless of hub, so it kept re-creating HostAssignments under the old hubSlug every morning — invisible in every UI view, but un-clearable from the new hub. One-shot heal migration `heal_orphan_standing_assignments_v1` deletes orphan rules + their future HostAssignments site-wide. Past HostAssignments stay as historical record. The valid-hubs set per program includes the primary `hostingHubSlug` AND every `ProgramCoverageHub` row (reviewer caught the initial primary-only implementation that would have wiped every legitimate AV/greeter rotation). Program-transfer PUT handler now wraps cleanup + `program.update` in a single `$transaction` so future transfers can't recreate the orphan state.

FK-Restrict pattern fix across four routes (`3117833`): after the heal landed and Jesse retested, the inline diagnostic surfaced `HTTP 500` on a different program's Reset. The reviewer-flagged `SubRequest.assignmentId` FK-Restrict bug was latent in four pre-existing production routes (`clear-rotations`, `release-host`, `assignments/[id]` DELETE, `assignments/reassign`) — all of which did `subRequest.updateMany` to cancel OPEN sub-requests then `hostAssignment.delete` to remove the parent. The cancel-then-delete pattern FK-violates the moment any non-OPEN SubRequest (CLAIMED, CANCELLED) references the row. All four routes replaced with the canonical SubClaim → SubRequest → HostAssignment `deleteMany` chain wrapped in `$transaction`, matching `/api/host/assignments/clear`. PATCH unclaim on `assignments/[id]` keeps the cancel-OPEN behavior because it only sets `userId = null` — no parent delete, no FK violation possible.
...
**Slice 2 — operational rollout.**  The `peer-led-silent-meditation` hub created via `/admin/hubs/new` with `assignmentGrantsTeacher: true` and `teacherLabel: "Facilitator"`. Good Morning and Good Evening Silent Meditation programs transferred to the hub via the ProgramEditor "Hosting & Access" tab.  Jesse added as coordinator using the new admin-bootstrap affordance (see below).  Tool registry default label renamed "Host Schedule" → "Scheduler" (the tool serves multiple hubs now; the host-team-specific name no longer reads correctly).  Manual chapter for the hub seeded via `prisma/seed-manual-peer-led-silent-meditation.mjs` + new `peer-led` group in `lib/manualGroups.ts`.

**ADMIN no longer bypasses hub content access (Slice 2).**  A hub is a team space; the team is defined by membership.  ADMIN configures hubs from `/admin/hubs` (still ADMIN-gated) but to interact with hub content — read or post conversations, view documents, claim sessions — an ADMIN must be a HubMember just like everyone else.  Matches GUIDING_TEACHER's existing behavior.  Changes: layout gate `hasAccess = isMember` (was `isMember || isAdmin`); 18 API route gates simplified from `!member && !isAdmin` to `!member`; `lib/hubAuth.ts` policy comment rewritten.  Preserved: `/admin/hubs` administration, hard-remove member at `/api/hub/[slug]/members/[userId]` DELETE, `effectiveCoordinator` / `requireCoordinator` / `canManageTrash` keep their ADMIN+GT bypass (coordinator-level authority, not access).

**"+ Add me as coordinator" admin affordance (Slice 2).**  Closes the catch-22 the bypass removal created: an admin who creates a new hub can't enter it (no membership) and can't add themselves via the in-hub members tab.  New endpoint `POST /api/admin/hubs/[slug]/add-me-as-coordinator` upserts a HubMember row for the calling ADMIN with `isCoordinator: true` + ACTIVE + hostingCapability + communicationsEnabled.  Idempotent: returns `alreadyCoordinator` / `promoted` / `created`.  Button rendered on `/admin/hubs/[slug]/edit` below the Coordinator section when the viewer isn't yet an active coordinator.

**Slice 2.5 — hub-scope every outbound email URL + welcome-email reliability.**  Slice 1 routed the data layer correctly but missed the email-URL layer; every link constructed in `lib/email.ts` was hub-agnostic, so multi-hub members (Nancy in both host-team and peer-led-silent-meditation) landed in default host-team view from any email — even when the email was about a peer-led session.  Two new helpers added to `lib/email.ts`:

- **`hubScopedUrl(path, hubSlug)`** for `/tools/*` paths.  Appends `?hub=<slug>` when slug isn't the host-team default (kept clean for the common case).  Auto-handles `?` vs `&` for paths that already have a query string.
- **`hubHomeUrl(hubSlug)`** for `/account/hub/<slug>` paths where the hub IS the path.

Every send* function in `lib/email.ts` now accepts `hubSlug` in its data interface; every callsite passes `program.hostingHubSlug ?? DEFAULT_HOSTING_HUB_SLUG`.  Welcome email fire-and-forget bug fixed: `/api/hub/[slug]/members` POST replaced bare `.catch(() => {})` (silently killed by Vercel teardown) with `after()` from `next/server`; `lib/syncHubMembership.ts` converted to awaited sends.  Errors now `console.error`'d instead of swallowed.

**Canonical CTA button helper.**  New `emailButtonHtml(label, url)` returns Outlook-safe inline-styled button HTML (table-centered, `#135274` background, white bold).  Every send* function passes a `{{*Button}}` variable (e.g. `{{coverButton}}`, `{{scheduleButton}}`, `{{hubButton}}`) alongside the existing plain URL variables.  Templates can be edited at `/admin/emails` to render the prominent button instead of the plain markdown link.  Migration `swap_email_cta_to_buttons_v1` does the swap conservatively (only where the canonical body pattern is still present); customized templates left alone with a notice in the deploy log.

**Email Template Gate policy refined.**  The "never `upsert`" framing was over-defensive.  Refined to: seed-only by default; intentional `update` with consent is fine, with per-template log output as the audit trail.  CLAUDE.md rule 3 + `RIM_Email_Engineering.md` document the nuance.

**Slice 2.6 — standing-rotation system generalized to hub-aware.**  Slice 1 deferred the standing-rotation routes; Slice 2 made the Rotations tab visible to coordinators of any hub via the hub-aware `isManager` flag, but the underlying API routes still hardcoded `host-team` for their capability gate.  Result: "shows up but doesn't work" inconsistency — the tab appeared clickable in peer-led-silent-meditation but any save would 403.  Slice 2.6 closes the gap.

All 6 standing-assignment API routes derive hub from the rotation's `programSlug` via `getProgramHubSlug`:
- `POST /api/host/standing-assignments` (create) — body.programSlug
- `GET .../standing-assignments?hub=&programSlug=` — `?hub=` (defaults host-team) or programSlug's hub if given.  Auth follows the program's hub when programSlug is filtered, not the `?hub=` param (precedence rule documented in `RIM_Scheduler.md`).
- `POST .../apply` — per-program apply gated by program's hub; apply-all (no programSlug) requires HOST_MANAGER or ADMIN
- `POST .../preview` — same pattern
- `POST .../release-host`, `POST .../end-bundle` — body.programSlug
- `DELETE .../[id]` — rotation lookup, derives hub from rotation.programSlug

New shared helper `lib/hubAuth.ts::isHubCoordinator(userId, hubSlug)` replaces inline hardcoded-hub coordinator lookups.  Standing-assignment emails (`sendStandingAssignmentScheduledEmail` / `Replaced` / `Released`) accept optional `hubSlug` and use `hubScopedUrl` for the schedule link.  UI plumbing: schedule page passes `hubSlug` → `HubScheduleClient` → `RotationsClient` → `?hub=<active>` on the rotation-list fetch.  One follow-up fix (`89051f3`): the view-tab strip in `HubScheduleClient.tsx` was gated by `isHostManager` (global role) instead of `isManager` (hub-aware), hiding the Rotations tab from hub coordinators who weren't also HOST_MANAGERs.  API capability and UI visibility now match.

**Engineering reference docs (Slice 2.5).**  Three new modular reference docs grounded in the work this session.  Distinct from the existing System/Hub/Role design docs which describe *what the system is*; these describe *how to work on it without breaking it*:

- **`RIM_Hub_Engineering.md`** — cross-cutting rules for hub-touching code.  The four routing layers, helpers, ADMIN policy, common pitfalls, grandfather policy, closing-ritual checklist.
- **`RIM_Email_Engineering.md`** — rules for outbound email code.  Template gate, URL construction, `after()` pattern, CTA convention, env-var trimming, template variable conventions.
- **`RIM_Scheduler.md`** — per-tool reference for `/tools/schedule` and its supporting routes.  Hub-scoping at every layer, sub-request flow, standing rotations with the full auth table, what's deferred.

CLAUDE.md Design Orientation table updated with three new rows pointing each task type at the right doc.  Closing ritual gets four new steps: 4b (engineering-doc updates), 4c (hub audit on hub-related slices), 4d (per-tool engineering doc creation when a slice touches a tool without one — self-perpetuating mechanism that grows the docset organically).

**Files touched (cumulative across Slice 2 + 2.5 + 2.6):** `lib/email.ts`, `lib/hubAuth.ts`, `lib/syncHubMembership.ts`, `lib/toolRegistry.ts`, `lib/manualGroups.ts`, `/api/admin/hubs/route.ts` + `[slug]/route.ts` + `[slug]/add-me-as-coordinator/route.ts` (new), `/api/hub/[slug]/members/route.ts`, `/api/host/sub-requests/*`, `/api/host/assignments/*`, `/api/host/standing-assignments/*` (all 6 routes), `/api/programs-pg/route.ts`, `app/admin/hubs/[slug]/edit/page.tsx`, `app/account/hub/[slug]/layout.tsx`, `app/tools/schedule/page.tsx`, `components/HubAdminForm.tsx`, `components/HubScheduleClient.tsx`, `components/RotationsClient.tsx`, plus 18 hub content API routes for the ADMIN-bypass removal.  Three new engineering docs: `RIM_Hub_Engineering.md`, `RIM_Email_Engineering.md`, `RIM_Scheduler.md`.  Two new migration entries (Scheduler label rename, CTA button swap) + the manual chapter seed.  CLAUDE.md updated for Design Orientation table + closing ritual.  Commits: `aba2e60`, `47141e2`, `5a7c7ed`, `cccf020`, `dafc409`, `3fba168`, `fdb441d`, `86ce52e`, `e446213`, `a80dc17`, `03f8537`, `51d1207`, `d5ad0fc`, `89051f3`.
...
### Program offering KIND — explicit, on the category (session 137)

A Program's *kind* (what the offering **is**) used to be inferred from a tangle of `registrationEnabled` + `programFormat` + recurrence. Session 137 makes it explicit on **`ProgramCategory.kind`** — a program inherits its category's kind. The category **name** stays editorial (the public-page heading); `kind` is the stable, behavior-driving code from `lib/programKind.ts`: `DROP_IN`, `COMMUNITY_GROUP`, `CLASS`, `EVENT`, `RETREAT`, `SERVICE`, `PRIVATE`. Kind answers *what it is*; registration (`registrationEnabled`) stays the separate "what registering does" signal. Behavior = both together, via `isOpenlyDroppable(kind, registrationEnabled)`.

- **Coordinators manage categories entirely in the category manager** (`/tools/programs/categories`): inline rename (saves on blur), set kind (dropdown), reorder (↑↓), add (name + kind), delete (× — guarded when programs are assigned). One complete home. The categories API backs all of it (`POST` accepts kind, `PATCH` for name/kind, `DELETE`, `/reorder`). The ProgramEditor's Categories tab keeps the per-program category *picker* and links out to the manager — its old inline add/delete (`CategoryOrderInline`, which lacked kind + rename) was removed so there's a single source of truth.
- **Dashboard placement** keys off kind: "Today"/community schedule shows a public Join only for openly-droppable kinds (`DROP_IN`; `COMMUNITY_GROUP` when open); commitments (`CLASS`/`EVENT`/`RETREAT`/`SERVICE`/`PRIVATE`) appear in "Coming up for you" once registered. The member program-detail gate uses the same predicate (so a drop-in with registration-as-enrichment, e.g. Essential Dharma Study, stays reachable; a commitment redirects to register).
- **Migration `add_program_category_kind`** backfilled the 6 live categories, split "Community Groups & Events" → **Community Groups** + **Events**, added a hidden **Private Sessions** category, and reassigned Day of Mindfulness + Bookmarks & Breath → Events and Private Teacher Meetings → Private Sessions.
- **Why category-carries-kind, not a parallel `programType`:** restraint (one concept, not two) + the category was already the thing coordinators pick. Several categories can share a kind (two drop-in categories both `DROP_IN`). A category is one kind; a single editorial section spanning multiple kinds (the old "Community Groups & Events") splits. Full rationale + the two-axis model: `RIM_Offering_Model.md`.
