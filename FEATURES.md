...
- **Coordinators** set a category's kind in the category manager (`/tools/programs/categories`) — a dropdown per row; the categories API gained a `PATCH` (and POST now accepts kind). New-category creation in the ProgramEditor doesn't set a kind yet (defaults null, safe fallback); kind is set on the manager. Deferred.
