...
- The three `/login` pages still wear **legacy Webflow class names** re-implemented in `custom.css` (they are not yet rebuilt on the `pp-` grammar). **Session 173** fixed two layout defects there, both reported by Jesse on the code-entry page: the shared wrapper ran 48px past the viewport and pushed the sign-in card 24px off centre at any width under ~1148px, and a member's own address — echoed back inside a `<strong>` and unbreakable as a single token — ran off the right edge past ~34 characters (26px for a 38-character address, 392px for a 77-character one). Both are `custom.css` fixes; the email value is user-supplied, so anything new that echoes it needs `overflow-wrap`.
- **See:** `RIM_Auth.md` (flow, code generation, error states, membership-existence check, rate-limit thresholds; pitfall 6 = the legacy-class layout traps).
...
> The next major build area — the rebuild began in earnest **session 148** (warm three-shade palette + program-detail redesign + the flush-nav decision) and the static front-facing pages followed in **session 169** (the `pp-` grammar). The design system — palette, the card-lift / recede-panel surface language, the `pp-` grammar, and the tombstones (the reverted floating nav, chapter band, and hero paper panel) — lives in **`RIM_Public_Pages.md`**; read it before any public-page UI/CSS work.
>
> **An offline clone of the old Webflow site** lives at `~/Sites/rim-site-archive/` (317 pages, 158 assets, 84 MB, captured 2026-08-07, outside the repo). It is the only copy of those assets outside Webflow. Measure against it — or better, against the live page — before rebuilding anything else.
>
> **The words are a ratified standard too, not just the pixels (session 174).** Public copy follows the house voice — authority is the `/how-jesse-writes` skill plus `RIM_Public_Pages.md` → "Copy and voice". Read both before changing any community-facing text, including button labels, empty states, and error strings. Non-negotiables: never **"free"** as a price word (RIM's offerings are *freely offered* and *community-supported*), never name the epoch, no em-dashes in prose, one image per page. **All copy implemented in session 174 is provisional until Jesse's read-aloud** — ratification is his explicit yes, never inferred from silence.

- **Community programs listing** — `/community-programs`. The public catalog of offerings, grouped by `ProgramCategory` (the taxonomy is editorial — changed in Program Manager, not in code). **Rebuilt session 170** onto the shared listing grammar: `pp-hero`, `.pl-card` rows that run the full container width and split into what-it-is (title + tagline) / when-it-is (schedule + format) / arrow, a 96 / 36 / 24 spacing cadence, and a closing membership block that speaks RIM's **dana** framing rather than "membership is free," linking to `/donate#dana-at-rim`. A hardcoded "Good first visit" badge (two slugs in the page file, never data) was removed in the same session. **Session 172:** one-time upcoming programs render the **date-led `.pl-card--time` sibling `.pl-card--date`** ("Sep 10–13" leads); concluded one-time programs auto-hide (`hideWhenPast`, see Programs); each category section carries `id={category.slug}` so the home page's doors deep-link into it.
...
- **This Week schedule** — `/this-week`. Dynamic weekly schedule (Mon–Sun, `?week=next`), shared occurrence logic with the Scheduler. **Rebuilt session 170** onto the same grammar as the programs listing: `pp-hero` (it had carried a third, bespoke hero implementation), and `.pl-card--time` rows where the time leads — matching the Scheduler's occurrence-first agenda. It had been styling public rows with `.lr-row`/`.lr-btn`, member-area components shared with `HubScheduleClient`, and referencing `.pl-list`, a class with zero rules. **Today is marked and reachable** (inverted date block, "Today" pill, `#today` jump in the hero); previously it sat ~2,360px down on desktop. Each row link now carries its own descriptive name in place of 17 identical "Learn More" links.
...
- **Join / membership threshold** — `/join`. The new-member door (see Auth). **Session 174** added a **"What membership means"** section ahead of the agreements, on Jesse's ruling that membership and the word *community* are themselves intimidating and must be disarmed before the agreements teach anything: no dues, no attendance requirement, nobody tracking how often you come — and the reader's own objection ("will I have to be social?") raised at the moment it occurs and answered. The agreements, their lead-in, and the form lead live in **`lib/communityAgreements.ts`** and render on `/join`, `/account/welcome`, **and the program registration form** — edits there reach all three.
- **Home** — `/`. **Rebuilt session 169** onto the `pp-` grammar; **composed session 172**: the image splits alternate sides (right/left/right); the **program doors are dynamic** — the live `ProgramCategory` taxonomy (Program Manager's rows, sortOrder), each with a kind-derived public line + offerings count, deep-linking to `id={category.slug}` anchors on `/community-programs` (empty categories excluded; page is `force-dynamic`); the doors chapter uses the split grammar (words left, doors right as Light-Pampas insets); the **Dana section is the third split** with the held-lotus photo (Olga Nayda, Unsplash). Looping bodhi-leaf video hero (dark, left-aligned — Jesse's deliberate A/B departure from the live site); the hero prefers the **MP4** source (flaky VP9 decode produced intermittent macroblocking; both transcodes verified clean frame-by-frame and byte-identical to the live Webflow files — the remaining 720p-on-Retina softness needs the original pre-transcode clip from Webflow's Assets panel).
- **What and how we practice** — `/what-we-practice` (**new session 174**; nav label "Our Practice", page title "A Handful of Leaves"). The page that answers what RIM is. Five signposted sections on the `pp-` grammar, pure prose on the ground: the story of the name · why a handful · how it is ordered · one practice, many doors · for anyone. **The load-bearing point: the handful is an *ordered structure*** — organized **by function** into seven gatherings, not by school or country — **and it exists as a response to** having every tradition available at once ("exposure everywhere, orientation nowhere"). It is not eclectic gathering and not picking and choosing; an earlier draft that said "gathered from across the traditions" and stopped read as a spiritual supermarket and was rewritten at Jesse's correction. Authority is the **community introduction** (`A Handful of Leaves: An Introduction`), the document given to every new participant — not currently in the repo. Carries exactly one image (the hall and the orchestra) and does **no shame-disarming** by design, because its reader has not walked in yet. Uses `.pp-prose--spine`.
- **Your First Visit** — `/your-first-visit` (**new session 174**). What walking in is actually like, for the person who has decided to come: drop-in means drop-in, chairs and cushions both, nobody will ask you to speak, come in late anyway, and a hard season needs no explanation. Linked from the home hero's **primary** button, from Programs ▾, and from `/what-we-practice`. **Two paragraphs are provisional placeholders** awaiting Jesse's own logistics (parking + entrance; shoes/tea/restrooms) — marked `PROVISIONAL` in the source, written to assert nothing unverifiable about the building. Backlog `2026-08-10-002` + a standing reminder in `UP_NEXT.md`.
- **Content / static pages** — `/diversity`, `/donate`, `/kalyana-mitta/*` (3: community-groups-events, guidelines, application), `/volunteerism/volunteer` + thanks page. **All rebuilt session 169** onto the `pp-` grammar. Until then they wore Webflow-era class names with **no rules in `custom.css`** and rendered as bare document flow. Donate matches the live page by measurement (585px statement card, 54px heading, 720px note, 530px timeline cards, alternating contemplation spine) and keeps its three Givebutter widgets plus a standing phone/email fallback; its accordion is now native `<details>`. KM community-groups lists real Programs from the Community Groups category rather than a hardcoded list.
- **Nav & footer** — `components/Nav.tsx` (sticky **flush** full-width white bar, CSS-only dropdowns, `isMemberArea`/`isAdmin` aware), footer in the root layout. Public nav slimmed (session 148) to **Programs ▾ · Get Involved ▾ · Members ▾ · Donate** (Courses + Teachers dropped from the bar; "Member Area" → "Members"). **Session 174** added **Our Practice** as a flat top-level link ahead of Programs — the first non-dropdown item since the s148 slimming, on the existing `.nav__link` class — plus **Your First Visit** inside Programs ▾; both mirrored in the mobile menu. The label is short on purpose (the page carries the full "What and how we practice"): the bar has only ~156px of slack at 1024px. The footer's newsletter block ("Stay Connected") is **global to every public page**, so its copy is not a home-page concern. A floating-pill nav was tried and reverted — see the tombstone in `RIM_Public_Pages.md`. **Session 173:** every row of the phone menu (`.nav__mobile-link`) was 48px wider than the viewport — right padding, row border, and hover background clipped on every public page; a page-level sweep couldn't see it because the menu is closed. Now `border-box`.
...
**Session 172 — orientation + the interaction floor.** The member-bar carries a **"Main site"** link (→ `/`) on every member/admin/tool surface at every viewport (on phones it sheds the wordmark and profile name to fit); the account rail's "Home" is **"My Home"**, the hub rail's footer link is **"My RIM"**, and ToolsNav adds a "My RIM" link on hub-backed entry. On phones the account rail is a **slide-in drawer** identical to the hub drawer (hamburger "My RIM" bar; the horizontal scroll strip is gone). One appended CSS layer gives the whole member area its floor: `:focus-visible` on ~65 interactive selectors (the content layer had none), 44px touch targets, 14px interactive text / 16px inputs; every wide table scrolls in its own container at every width; and `box-sizing: border-box` on the layout containers + 16 swept classes (the no-global-border-box trap was clipping hub/tool/admin pages at half-screen widths). Nav.tsx's unreachable member-area nav branches were deleted.

- **Dashboard** — `/account/dashboard`. The member home: "Today" groups the single immediate/live offering from later sessions, with one balanced time/title/context/action row and no redundant “Zoom is open” message; "Coming up for you" gathers registered offerings into one quiet white list surface with clear date and status cells. An empty day says so instead of repeating the date, and dana invitations use words rather than an unexplained icon. Host early-entry and member-entry language are distinct. Placement keys off offering kind + registration (session 137); `DashboardAutoRefresh` handles state transitions. The account rail collapses on desktop and keeps long team names readable. A one-time, dismissible **host-welcome panel** (session 143, `HostWelcomePanel`) greets a pre-staged host who's just onboarded — shown when they belong to a hub, have hosting assigned, and haven't dismissed it (`User.hostWelcomeSeenAt`); links to their Scheduler view. Dismiss/follow → `POST /api/account/host-welcome-seen`.
...
- **Dated events retire themselves** (session 172) — `Program.hideWhenPast` (default true; Visibility-tab checkbox shown only for one-time programs). A one-time program (no `recurrenceFreq`, has `startDatetime` — derived from data shape, deliberately not from category kind) whose CT day has fully passed **leaves the public listings at read time** (`/community-programs` + the KM groups page, one shared rule `lib/programUtils.ts::hasConcludedOneTime`) and is **archived by the daily `archive-concluded-programs` cron** the next morning. On the catalog, an upcoming one-time program renders the **date-led `.pl-card--date`** ("Sep 10–13" leads, matching the occurrence-first grammar). The Archived tab in Program Manager reads by recency. Registration records survive the archive for registrar history and reporting, while `/session/[slug]/enter` refuses archived programs.
...
- **Routes/API:** `/api/registrations`, `/api/registrations/[id]`, `/api/registrations/[id]/decline-dana`, `/api/programs/[slug]/registrations` (CSV export), `/api/programs/[slug]/ical`, `/api/programs/[slug]/send-reminder`.
- **Registration changes:** cancellations are registrar-managed; members can update questionnaire responses only through a registrar-sent tokenized link (`/update/[token]` + `editToken` on `Registration`).
...
Hubs are team-centric workspaces. Each provides **one universal Home**, **Updates**, **Conversations**, **Files** (Google Workspace), **Members**, and coordinator-gated **Trash** views inside one shared hub rail. (The stable Updates URL remains `/activity`.) Home is stable core infrastructure: greeting, genuine first-visit welcome, a short conditional personal-attention list, pinned conversations, and coordinator-editable welcome/orientation. Passive chronological history stays off Home. A Space may have zero or more apps; when enabled registered apps exist, exactly one is primary. Supporting apps stay compact and custom links stay quiet. No app replaces Home. `HubType` ∈ OPERATIONAL / GOVERNANCE / COMMUNITY_GROUP; all managed at `/admin/hubs`.

**Per-hub feature switch.** `Hub.conversationsEnabled` (default true) is a per-hub Conversations toggle set in hub settings; every new hub gets the shared built-ins and best-effort Google Files provisioning, while apps are installed explicitly. *(The session-165 `Hub.openToAllMembers` "Community" open-to-all primitive was retired in session 166 — see the Removed table. Every Space is now a stewarded team/project with a real roster; `canAccessHub(member, roles)` is purely membership OR GT. The column drop is Phase 2, backlog `2026-07-16-001`.)*

- **Workspace routes:** `/account/hub/[slug]` + `/activity`, `/conversations`, `/conversations/[id]`, `/files`, `/members`, `/trash`. API under `/api/hub/[slug]/*`, `/api/hubs/[slug]/*`, and the gated `/api/files/*` system.
...
- **Conversation lifecycle** — Active → Archived → Trash (`deletedAt` plus the archived/closed state). Trash gated by `canManageTrash`; Google Files use governed propose→approve removal instead.
- **Updates** — `/activity`, a shared server projection of conversation, visible Google Files, member-join, and installed-app events. Every row names its source and event kind; File comments are distinct from Space Conversations. Filters are **All / Recent / For me**. Recent is a predictable rolling seven-day window: opening the page or an item never clears it, and All retains the complete history. A Scheduler-style View menu offers Date and time (default chronology), All categories (grouped), and each exact category active in that Space; exact category streams are filtered before server pagination. Passive Updates have no unread dot; durable attention remains owned by the app whose open/closed state gives it meaning.
- **App installation contract** — `lib/toolRegistry.ts` declares canonical path, a distinct semantic navigation icon, compatibility, primary eligibility, and promised Home/Updates/attention contributions; the exhaustive server provider registry in `lib/hubApps.ts` fulfills them. `HubAppLink.isPrimary` plus a partial unique DB index enforces one primary app per Space. Scheduler is multi-Space; Program Manager (`registrar`) and Course Manager (`courses`) are primary-Space only. Custom links are navigation only. New incompatible installs are blocked in UI + API, existing rows are grandfathered during ordinary edits, and app replacement is transactional.
...
### Hub admin
`/admin/hubs`, `/admin/hubs/new`, `/admin/hubs/[slug]/edit` — create/edit/archive hubs, compatible apps/custom links, coverage copy, schedule flags. ADMIN only. New hubs auto-write a coordinator `HubMember` for the creating admin and preserve the default-enabled Conversations switch sent by the form.
...
- **Occurrence-first agenda** (sessions 167–168) — every dated session leads with one calendar-style date/time block, then program, coverage state, and next action. The same grammar reaches Scheduler cards, rotation projections/saved confirmations, `/this-week`, and cross-hub staffing. Single-slot coordinator actions are gathered under Manage coverage; the greeter multi-claim people view stays distinct.
...
## Google Workspace Files (COMPLETE — RIM's document & file system, through session 166)
...
- **The model** — one Google Cloud **service account** (`rim-files@…`) is RIM's only Google identity, a Manager on each org-owned Shared Drive. **No member or volunteer gets a Google account.** RIM's database is the permission system; the service account performs every Drive operation server-side. Editing uses **link-as-key** (RIM hands the anyone-with-link only to authorized members — the accepted Zoom-style trade); reading happens **inside RIM** (Google Docs rendered from sanitized HTML export). **Drive folders are the filing system**, live-browsed so Drive is the source of truth and can't drift.
- **Storage topology (session 164)** — the service account **can't create Shared Drives** (probe: `403 userCannotCreateTeamDrives`), so a Space's storage is an **auto-created folder** in one shared **`RIM — Spaces`** container Drive. Community + any future sensitive Space keep their **own** Drive (the hard wall). `Hub.googleDriveId` + `googleRootFolderId` express both.
- **Read (Slice 2)** — the Finder (`FilesBrowser`): each Space's **Files** tab (the global `/account/files` finder was retired s165 — files live only per-Space); folders-first browse, breadcrumbs, phone drill-down; every file opens the **detail page** (`/account/files/[fileId]`, s166); streamed binaries.
- **Write (Slice 3, session 164)** — create Doc/Sheet/Slides/folder, rename, move, **move-to-trash** (Drive's 30-day trash; no permanent delete exposed), and **uploads ≤500 MB** (Blob staging → `GoogleFileTransfer` ledger → `after()` transfer + daily cron `process-file-transfers`). Two shared write gates. New API `/api/files/create`, `/api/files/[fileId]` (PATCH), `/api/files/upload`. Migration `google_file_transfers_v1`.
- **Per-folder access gate (session 164)** — authorization is subtree-aware so many Spaces safely share the container Drive: a file must descend from a Space's root folder (`fileWithinFolderRoot` / `resolvePlaceForFile`, fails closed). Isolation enforced on reads AND writes; the write path (hub PATCH) + drive picker refuse mapping a hub whole-drive onto a managed drive.
- **Auto-provisioning (session 164)** — new hubs get a Space folder on creation; existing hubs via one-click "Set up files for this team" (`/admin/hubs/[slug]/edit`). `provisionHubSpaceStorage`, idempotent.
- **Admin link revoke/lockdown (session 164)** — `/admin/google-files` (ADMIN): revoke a minted link or lock down a place; `lib/googleFileAdmin.ts`. Closes backlog `2026-07-14-001`.
- **The file-detail layer (session 166)** — `GoogleFileMeta` (sparse, loose-keyed): **creator attribution** (backfilled from the audit log; "Added directly in Google Drive" when unknown, never the service-account) + **draft/"held" state** (opt-out; RIM-created docs born held → "Your drafts" → Share). The **detail page** renders fidelity-aware (Google `/preview` iframe for shared docs, RIM export for drafts, native embed for PDF/image/audio) and hosts attribution + a **conversation per file** (`FileComment`, plain-text + reactions). **Governed deletion** — Remove is a proposal (`pendingDeleteAt`) a **Space lead** (`isSpaceLead` = GT/ADMIN/coordinator) approves (→ Google trash) or keeps; requester can cancel. **Basecamp notifications** — per-post, default No one (`NotifyPicker`; templates `file-shared`/`file-comment`). Orphan sweep cron for files deleted directly in Drive. Migrations `google_file_meta_v1`, `google_file_pending_delete_v1`. See `RIM_GoogleWorkspace.md` §10.
- **Access** — a Space's Files require ACTIVE membership or GUIDING_TEACHER; ADMIN-alone does not pass (session-128 boundary). Env: `GOOGLE_SERVICE_ACCOUNT_EMAIL`, `GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY` (server-only). *(The open-to-all Community drive was retired s166.)*
- **Deferred:** the literal `Hub`→`Space` code rename + the GT self-serve "create a Space" entry point; **cross-Space sharing** (backlog `2026-07-15-001`); the in-app notification inbox. See `RIM_GoogleWorkspace.md` §9–§10.
...
- **Crons (8, in `vercel.json`):** `send-reminders` (0 14 * * *), `cleanup-incomplete-accounts` (0 5), `apply-standing-assignments` (0 8), `cleanup-rate-limits` (15 10), `cleanup-pending-registrations` (30 5), `process-file-transfers` (0 9 — Blob→Drive upload backstop, s164), `sweep-orphan-file-data` (45 9 — clears RIM per-file rows for files permanently deleted directly in Drive, s166), `archive-concluded-programs` (15 9 — archives one-time programs whose CT day has passed, honoring `hideWhenPast`, s172). All validate `CRON_SECRET`.
...
- **Redirects (`vercel.json`)** — legacy and removed paths fold into current ones (`/account/registrar*` → `/tools/programs*`, `/account/hub/teacher*` → `/account/hub/courses*`, removed `/account/programs*` and `/account/dashboard-my-registrations` → `/account/dashboard`, `/account/dashboard-my-library` → `/account/courses`, …).
...
- **Member registration history/detail and self-service cancellation** — removed for the current simplified release. The dashboard remains the member's view of upcoming commitments and links to the public program detail; registration records, registrar tools, waitlists, dana, confirmations, reminders, calendar links in email, and registrar-managed cancellation remain active. Removed paths redirect to `/account/dashboard`.
...
| Community Space (open-to-all `Hub.openToAllMembers`) | s166 | An open, ownerless commons fit nothing in the coordinator-led governance model (no lead to approve removals; "notify everyone" = the whole sangha). Reversed the s165 Community-as-a-Space decision. Access primitive + Google-files Community place + seeded hub removed (Phase 1); `openToAllMembers` column drop is Phase 2 (`2026-07-16-001`). The "RIM — Community" Google Drive stays in Google | session-log s166; `RIM_GoogleWorkspace.md` §9 |
