...
**Three visible role pills.** `host` / `teacher` / `cohost` orthogonal flags in participant metadata. `host: true` ↔ Session Host (Host pill, teal). `teacher: true` ↔ ProgramTeacher (Teacher pill, warm gold — distinct dharma identity). `cohost: true` ↔ Co-host capability AND not Host AND not Teacher (Co-host pill, muted slate). Constraint enforced server-side (token routes) and client-side (RIMConference metadata-seeding effect) so the two sources of truth can't drift. A Session Host who is also a Teacher renders both pills. The Co-host pill only shows when neither of the other two applies, so each tile renders at most two pills.
