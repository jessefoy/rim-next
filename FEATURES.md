...
- **Dated events retire themselves** (session 172) — `Program.hideWhenPast` (default true; Visibility-tab checkbox shown only for one-time programs). A one-time program (no `recurrenceFreq`, has `startDatetime` — derived from data shape, deliberately not from category kind) whose CT day has fully passed **leaves the public listings at read time** (`/community-programs` + the KM groups page, one shared rule `lib/programUtils.ts::hasConcludedOneTime`) and is **archived by the daily `archive-concluded-programs` cron** the next morning. On the catalog, an upcoming one-time program renders the **date-led `.pl-card--date`** ("Sep 10–13" leads, matching the occurrence-first grammar). The Archived tab in Program Manager reads by recency. A member's registration history survives the archive: `/account/programs/[slug]` stays reachable for registrants (live affordances hidden — "This program has concluded."), and `/session/[slug]/enter` refuses archived programs.
