...
Hubs are team-centric workspaces. Each provides **one universal Home**, **Activity**, **Conversations**, **Files** (Google Workspace), **Members**, and coordinator-gated **Trash** views inside one shared hub rail. (Native Documents and Mind Maps were both retired — sessions 165/165.) Home is stable core infrastructure: greeting/attention state, genuine first-visit welcome, pinned conversations, a recent-Activity preview, and coordinator-editable welcome/orientation. Enabled `HubAppLink` installations add cards/modules on top; no hub gets a parallel Home implementation. `HubType` ∈ OPERATIONAL / GOVERNANCE / COMMUNITY_GROUP; all managed at `/admin/hubs` (the source of truth for the full set — ~16 hubs). Scheduler-using hubs include **host-team**, **peer-led-silent-meditation**, **audio-visual**, and **greeter**; Program Manager belongs to **registrar** and Course Manager to **courses**.
...
- **Activity stream** — `/activity`, a shared server projection of conversation, visible Google Files, member-join, and installed Scheduler events. Page/API share `lib/hubActivity.ts`; pagination works from the initial server page; “My activity” means authored by me everywhere. `HubMember.activitySeenAt` keeps the Activity badge independent from Home/conversation visits.
- **App installation contract** — `lib/toolRegistry.ts` declares canonical path, `multi-space` vs `primary-space`, and Home/Activity adapter keys; `lib/hubApps.ts` composes enabled installations into Home. Scheduler is multi-Space; Program Manager (`registrar`) and Course Manager (`courses`) are primary-Space only. Custom links are navigation only. New incompatible installs are blocked in UI + API, existing rows are grandfathered during ordinary edits, and app replacement is transactional.
