...
- **Updates** — `/activity`, a shared server projection of conversation, visible Google Files, member-join, and installed-app events. Every row names its source and event kind; File comments are distinct from Space Conversations. Filters are All / New / For me. A Scheduler-style View menu offers Date and time (default chronology), All categories (grouped), and each exact category active in that Space; exact category streams are filtered before server pagination. `HubMember.activitySeenAt` keeps the quiet Updates-new dot independent from Home/conversation visits.
