...
| 2026-03-16 (session 58) | **Session tab state machine fixes.** **(1) Live state re-evaluation interval:** `computeState` was called once at component mount and never updated. Added `useState` tick counter incremented inside the existing 60-second poll interval — triggers re-render so `computeState` re-runs with fresh `Date.now()` on every tick. The tab now correctly transitions through all 6 states (later-today → getting-ready → live → post-session → done) without a page reload. **(2) Removed server-side `sessionEnded` from `isEnded`:** The server-computed `prog.sessionEnded` boolean was frozen at page render time and could force `isEnded = true` even when the session was still live. Removed from `isEnded` OR condition — `manuallyEnded` (explicit Close Session action) and `timeEnded` (real-time `Date.now()` check) are the only two reliable paths. Key files: `components/SessionLiveClient.tsx`, `app/account/hub/[slug]/session/page.tsx` (no change needed there — fix is entirely in client component). |
