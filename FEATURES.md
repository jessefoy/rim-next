...
| `Teacher` | `name`, `slug`, `bio Json?`, `photoUrl?`, `isActive` | ✅ Built (profile infra exists; standalone profile record — no lessons relation) |
| `LessonTeacher` | `id`, `lessonId`, `userId` (→ User), `order Int`, `@@unique([lessonId, userId])`, `@@map("lesson_teachers")` | ✅ Migrated session 65 (was Teacher-based; now User-based via userId) |
...
- **LessonTeacher model (session 65):** Migrated from `Teacher` model (`teacherId String`) to `User` model (`userId String`). All teacher-related admin routes and pages updated to remove the now-dropped `lessons` relation from `Teacher`. `/api/members/search?q=` added for TEACHER/ADMIN member name lookup. LessonEditor teacher picker now points to this route. Lesson page + series page display teacher names from `lt.user.firstName/lastName`. Series page shows byline below series title.
