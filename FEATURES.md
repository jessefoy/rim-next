...
**Per-hub feature switch.** `Hub.conversationsEnabled` (default true) is a per-hub Conversations toggle set in hub settings; every new hub comes up fully equipped (all features + apps) with Google Files auto-provisioned on create. *(The session-165 `Hub.openToAllMembers` "Community" open-to-all primitive was retired in session 166 — see the Removed table. Every Space is now a stewarded team/project with a real roster; `canAccessHub(member, roles)` is purely membership OR GT. The column drop is Phase 2, backlog `2026-07-16-001`.)*
