/**
 * update-manual-host-schedule-v4.mjs — Adds "For virtual and hybrid sessions"
 * section to the host-schedule chapter.
 *
 * v4 (session 112):
 *   Adds a short section explaining the "Enter room →" link that appears on
 *   every upcoming virtual/hybrid session row. Hosts can enter the room at any
 *   time — before a session to test audio, or 10-12 minutes early to hold the
 *   welcoming space as participants arrive. Placed between "What you see when
 *   you arrive" and "The four buttons you might see."
 *
 * Idempotent at the record level (update by slug). Wired into migrate.mjs
 * behind the one-time flag 'update_manual_host_schedule_v4'.
 */
...
<h2>Getting there</h2>
<p>Sign in to your RIM account. From your home, open the <strong>Host Hub</strong> and click <strong>Host Schedule</strong> in the left sidebar. The full address is <strong>/tools/schedule</strong> — useful if you want to bookmark it on your phone.</p>
...
export async function updateManualHostScheduleV4(db) {
  ...
  if (existing) {
    ...
    console.log("  ✔ Updated manual section: host-schedule (v4)");
  } else {
    await db.manualSection.create({
      data: { slug: "host-schedule", order: 7, ...data },
    });
    console.log("  ✔ Created manual section: host-schedule (v4)");
  }
}
