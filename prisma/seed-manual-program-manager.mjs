...
export async function seedManualProgramManager(db) {
  ...
  const body = [
    ...
    p("Click ", b("+ Add Program"), " to open the editor. It has seven tabs, each focused on one part of the setup:"),
    sp(),
    p(b("Content"), " — The basics. Give it a name (the title everyone sees), and the slug will auto-generate. Add a tagline, description, image, pull quote, and assign teachers."),
    sp(),
    p(b("Schedule"), " — When and where. Set the format (in-person, virtual, hybrid), venue, start/end dates, and recurrence pattern. The Schedule Label and Time Label auto-generate from these fields — clear either one to regenerate, or type to override. For virtual programs, you can enable Open Access to generate a guest join link."),
    sp(),
    p(b("Categories"), " — Pick which section this program appears under on the public Programs & Events page. You can also reorder categories from here."),
    sp(),
    p(b("Registration"), " — Who can sign up and what they see. Enable registration, set capacity, add custom questions, and write your confirmation and reminder messages."),
    sp(),
    p(b("Dana"), " — How donations work. Choose a mode (None, Voluntary, Base + Dana, or Fixed), set amounts, and write the messaging shown during registration and on the public page."),
    sp(),
    p(b("Dashboard"), " — What members see on their dashboard card. Add a special announcement or an early arrival message."),
    sp(),
    p(b("Visibility"), " — Where this program shows up. Control sort order, and hide from the public listing or member dashboards."),
    sp(),
    ...
    p("Set a reminder date and message in the program editor (Registration tab). When the date arrives, go to the registration detail page and click ", b("Send to Remaining"), ". You can also send individual reminders by expanding a row and clicking Send Reminder."),
    sp(),
    ...
    sp(),
    p("To assign a category, go to the Categories tab in the editor and pick one from the dropdown."),
    sp(),
