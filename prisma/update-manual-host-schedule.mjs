 ...
 *   2. Names Maria explicitly (matching the rewritten host-hub chapter)
 *      instead of "the host coordinator" generically.
...
<p><strong>One important thing:</strong> until someone covers, you're still officially the host. If nobody picks it up, you're expected to host. Please plan accordingly — if it's urgent, also reach out to Maria directly so she knows.</p>
...
<p>None of the actions on this page are irreversible. Every confirmation can be cancelled before it happens. After the fact, you can ask the team to cover, cancel a request, or reach out to Maria to fix things directly. Mistakes are fine.</p>
<p>If anything is confusing — a button you can't find, a label that doesn't make sense, a workflow that feels harder than it should be — tell Maria. The page is meant to make hosting easy. If it doesn't, that's worth fixing.</p>
