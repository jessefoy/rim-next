...
<p><strong>Teacher</strong> (warm gold). Anyone listed as a teacher of the program. This pill rides alongside the Host pill if the same person holds both roles; otherwise it stands on its own. The Teacher pill also unlocks the bell-friendly audio setting (see Bell mode below). Some programs use a different word for this role — peer-led silent sits may show <strong>Guide</strong>, recovery-tradition offerings may show <strong>Facilitator</strong>, skills-based classes may show <strong>Instructor</strong>. The pill's color and meaning don't change; only the label varies per program. A coordinator can set this on the program record.</p>
